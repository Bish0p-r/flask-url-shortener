...
def test_shortener_post(client, user):
    ...
    response = client.post(url_for('api.url_shortener_url_shortener'), json={"original_url": "https://google.com"})
    ...
    response = client.post(url_for('api.url_shortener_url_shortener'),
...
def test_auth_post(client):
    request_data = {"email": "test@test.com", "password1": "testtest", "password2": "wrongpassword"}
    response = client.post(url_for('api.auth_register'), json=request_data)

    assert response.status_code == 400
    assert b"Passwords don't match" in response.data

    request_data = {"email": "test@test.com", "password1": "testtest", "password2": "testtest"}
    response = client.post(url_for('api.auth_register'), json=request_data)
    data = json.loads(response.data.decode())
    ...
    response = client.post(url_for('api.auth_login'), json={"email": "test@test.com", "password": "wrongpassword"})

    assert response.status_code == 400
    assert b"Invalid email or password" in response.data

    response = client.post(url_for('api.auth_login'), json={"email": "test@test.com", "password": "testtest"})
    data = json.loads(response.data.decode())
